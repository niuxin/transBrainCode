...
parser.add_argument('roadName', help='tls name')
parser.add_argument(
...
file_name = args.roadName
f_input = open(file_name + '.txt', 'w')
...
for keyIndex, key in enumerate(allKeys):
    ...
    f_input.write('    <tlLogic id="' + key + '" type="static" programID="' +
          subkey + '" offset="' + offset + '">\n')
    for p in params:
        print('        <param key="' + p[0] + '" value="' + p[1] + '"/>')
        f_input.write('        <param key="' + p[0] + '" value="' + p[1] + '"/>\n')
